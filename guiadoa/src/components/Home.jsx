...
import ColorTextBuilder  from './ColorTextBuilder.jsx';
...
// Ferramentas extras — abrem modal interno (não mudam de rota)
const EXTRAS = [
  { id: 'color_builder', icon: '🎨', title: 'Texto Colorido', sub: 'Builder de cores', cor: '#9B59B6' },
];
...
const Home = ({ setRoute }) => {
  ...
  const [modalExtra,  setModalExtra]  = useState(null); // 'color_builder' | null
            ...
            {EXTRAS.map((tool, i) => (
              <button
                ...
                style={{
                  ...
                  overflow: 'hidden',
                  position: 'relative',
                  animation: `tool-in 0.3s ${0.18 + i * 0.04}s ease both`,
                  transition: 'transform 0.12s, box-shadow 0.12s',
                }}
                onMouseDown={e  => { e.currentTarget.style.transform = 'scale(0.95)'; e.currentTarget.style.boxShadow = 'none'; }}
                onMouseUp={e    => { e.currentTarget.style.transform = 'scale(1)';    e.currentTarget.style.boxShadow = `0 4px 18px ${tool.cor}30`; }}
